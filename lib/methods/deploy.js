...
function deployServerInNewTab() {
  ...
  var command = this.ttabDir + ' tpt -d --server ' + server.name + ' --type ' + type.name;
...
function getUsedPorts() {
  ...
  if (!run) return;
  var command = 'python ' + app.pythonBinDir + ' ports --docker ' + run.dockerName;
...
function getRegisterDockerCommand(config) {
  ...
  return ['python ' + app.pythonBinDir + ' register ' + serviceYamlPath, 'python ' + app.pythonBinDir + ' register ' + controllerYamlPath, 'cd ' + project.dir].join(' && ');
}
