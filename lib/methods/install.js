...
function installDocker() {
  ...
  var scopeDockerVersionDigit = parseInt(scope.dockerVersion.replace(/(\.+)/g, ''));
  if (dockerVersionDigit > scopeDockerVersionDigit) {
    var dockerFile = 'docker-' + scope.dockerVersion;
    var command = ['exec wget https://get.docker.com/builds/Darwin/x86_64/' + scope.dockerVersion, 'cp ' + dockerFile + ' $(which docker)', 'rm ' + dockerFile].join(' && ');
...
function getInstallKubernetesCommand() {
  ...
  var commands = ['cd ' + _path2.default.join(app, 'bin')];
...
function installInApp() {
  ...
  // write
  var writtenConfig = Object.assign({}, app.config);
...
function installBasePlaceholderFiles() {
  ...
  program.image = 'base';
  program.method = null;
  program.methods = [].map(function (newProgram) {
    return function () {
...
function installServer() {
  ...
  var installCommand = 'cd ' + server.dir + ' && ' + program.permission + ' sh scripts/' + fileName;
...
function installPlaceholderFile() {
  ...
  var templateFolderDir = program.folder === 'server' ? server.scopeServerDir : _path2.default.join(server.scopeServerDir, program.folder);
