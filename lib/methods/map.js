...
exports.getArrayFromArrayOrObject = getArrayFromArrayOrObject;
exports.map = map;
...
function getArrayFromArrayOrObject(value) {
  return Array.isArray(value) ? value : (0, _lodash.values)(value);
}
...
function map() {
  ...
  var methods = (program.method ? [typeof program.method === 'string' ? this[program.method] : program.method] : program.methods.map(function (method) {
    return typeof method === 'string' ? _this[method] : method;
  })).filter(function (method, index) {
    var isDefined = typeof method !== 'undefined';
    if (!isDefined) {
      _this.consoleWarn('no such method ' + program.methods[index]);
    }
    return isDefined;
  });
  ...
  var names = collectionSlugs.map(function (collectionSlug) {
    // is the slug an array ?
    if (collectionSlug.slice(-1)[0] === ']') {
      return collectionSlug.match(/\[(.*?)\]/g).slice(-1)[0] // take the last match
      ...
      */
      ...
      var key = collectionSlug;
      // collection = getArrayFromArrayOrObject(get(this, key))
      return Object.keys((0, _lodash.get)(_this, key) || {});
    }
  });
  ...
  var singularNames = collectionSlugs.map(function (collectionSlug) {
    var pluralName = collectionSlug.split('.').slice(-1)[0].split('[')[0];
    if (pluralName.endsWith('ByName')) {
      pluralName = pluralName.slice(0, -6);
    }
    return (0, _pluralize2.default)(pluralName, 1);
  });
