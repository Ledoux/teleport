...
function setAppEnvironment() {
  ...
  if (!app.config.scopesByName.default.dir) {
    app.config.scopesByName.default.dir = _path2.default.join(app.dir, 'default');
...
function setProjectEnvironment() {
  ...
  if (program.venv !== 'false') {
    project.config.pip = _path2.default.join(project.dir, 'venv/bin/pip');
  } else if (typeof project.config.pip === 'undefined') {
    project.config.pip = _child_process2.default.execSync('which pip').toString('utf-8').trim();
  }
}
