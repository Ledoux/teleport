...
function setBackendEnvironment() {
  ...
  backend.subDomainName = (0, _utils.toDashCase)(backend.siteName);
  backend.capitalUnderscoreSubdomainName = (0, _utils.toCapitalUnderscoreCase)(backend.subDomainName);
...
function setRunEnvironment() {
  ...
  if (type.hasDns) {
    ...
    // subdomain
    var subDomainName = '' + dnsPrefix + backend.subDomainName;
    if (!server.isMain) {
