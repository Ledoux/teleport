...
exports.bundlerStart = bundlerStart;
...
function bundlerStart() {
  var app = this.app,
      project = this.project;

  if (!_fs2.default.existsSync(_path2.default.join(project.dir, 'bundler'))) return;
  this.consoleInfo('Let\'s start the bundler');
  var command = app.ttabDir + ' \'cd ' + project.dir + ' && sh bin/localhost_bundle.sh\'';
  this.consoleLog(command);
  console.log(_child_process2.default.execSync(command).toString('utf-8'));
}