...
def get_ports(docker):
    """
    Get ports for all services related to an infra server
        - docker: the docker name ('prod2', 'dev3', ...)
...
def get_used_ports(docker):
    res = get_ports(docker)
    used_ports = []
...
def get_available_ports(docker):
    available_ports = list(set(range(9700, 9800)) - set(get_used_ports(docker)))
